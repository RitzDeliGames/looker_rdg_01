...
view: transactions {
  ...
  dimension: iap_revenue {
    ...
    sql: CASE WHEN REPLACE(JSON_EXTRACT(${extra_json},"$.transaction_purchase_currency"),'"','') = "CURRENCY_01" THEN CAST(REPLACE(JSON_EXTRACT(gaming_block_events.extra_json,"$.transaction_purchase_amount"),'"','') as NUMERIC) END AS gaming_block_events_iap_revenue ;;
  }
